...
namespace App\Log\Register;

class DailyCronReport extends \App\Log\Register
{
    ...
    protected function taskToArray(\App\Scheduling\Task\TaskInterface $task, $completed = true)
    {
        return array("id" => $task->id, "name" => $task->getName(), "output_ids" => $task->getLatestOutputs()->pluck("id"), "completed" => $completed);
    }
    public function start()
    {
        $dateTime = \App\Helpers\Carbon::now()->toDateTimeString();
        $this->write(array("startTime" => $dateTime, "tasks" => array(), "finishTime" => $dateTime));
    }
    public function finish()
    {
        $current = $this->getValue();
        $current["finishTime"] = \App\Helpers\Carbon::now()->toDateTimeString();
        $this->write($current);
    }
    public function completed(\App\Scheduling\Task\TaskInterface $task)
    {
        $current = $this->getValue();
    ...
    public function notCompleted(\App\Scheduling\Task\TaskInterface $task)
    {
    ...
    public function taskDataToHtml(array $taskData)
    {
        ...
        if (!empty($taskData["output_ids"])) {
            $task = \App\Scheduling\Task\AbstractTask::find($taskData["id"]);
            if (!$task) {
                return "";
            }
            if ($task instanceof \App\Cron\Task\CheckForWhmcsUpdate) {
                return "";
            }
            ...
            foreach ($taskData["output_ids"] as $id) {
                $output = (new \App\Log\Register())->where("id", $id)->first();
                if (!$output) {
                    continue;
                }
    ...
    protected function updateAvailableInfoAlert($data)
    {
        ...
        //             $task = \App\Scheduling\Task\AbstractTask::find($taskDetail["id"]);
        //             if (!$task || !$task instanceof \App\Cron\Task\CheckForWhmcsUpdate) {
        //                 continue;
        //             }
        ...
        // }
        // return $html;
        return "updateAvailableInfoAlert";
    }
    ...
    protected function viewSummaryLink()
    {
        ...
        return url(config("app.url_admin")."/automationstatus.php?date=" . date("Y-m-d"));
    }
