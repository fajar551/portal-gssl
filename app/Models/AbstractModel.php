...
class AbstractModel extends \Illuminate\Database\Eloquent\Model
{
    ...
    {
        parent::boot();
        // TODO: self::observe(new Observer());
    }
    ...
    protected function hasColumn($column)
    {
        ...
        static::$tableColumnCache[$this->table]["columns"] = array_map("strtolower", DB::getSchemaBuilder()->getColumnListing($this->table));
        if (in_array($column, static::$tableColumnCache[$this->table]["columns"])) {
            return true;
        }
    ...
    public function getAttribute($key)
    {
        ...
        if ($isColumnMapped && in_array($originalKey, $this->getDates())) {
            ...
            if (in_array($dateValue, array("0000-00-00 00:00:00", "0000-00-00")) || empty($dateValue)) {
                $value = \App\Helpers\Carbon::createFromTimestamp(0, config('app.timezone'));
            } else {
        ...
        if (is_null($value)) {
            $value = parent::getAttribute($this->snake_case($key));
        }
        if (is_null($value)) {
    ...
    public function setAttribute($key, $value)
    {
        ...
        if (!$this->isAttributeSet($key)) {
            if ($this->isAttributeSet($this->snake_case($key)) || $this->hasColumn($this->snake_case($key))) {
                $key = $this->snake_case($key);
            } else {
    ...
    // }
    public function fromSemanticVersion($version)
    {
        // return $version->getCanonical();
        return "";
    }
    ...
    public static function convertBooleanColumn($column)
    {
        ...
        DB::table($table)->where($column, "off")->update(array($column => ""));
        DB::table($table)->where($column, "!=", "")->where($column, "!=", "0")->update(array($column => 1));
        DB::table($table)->where($column, "")->update(array($column => 0));
        DB::connection()->getPdo()->exec("alter table `" . $table . "` change `" . $column . "` `" . $column . "` tinyint(1) not null");
    }
    public static function convertUnixTimestampIntegerToTimestampColumn($column)
    {
        ...
        DB::getSchemaBuilder()->table($tableName, function ($table) use($tempColumn) {
            $table->timestamp($tempColumn);
        });
        $pdo = DB::connection()->getPdo();
        $statement = $pdo->prepare("update `" . $tableName . "` set `" . $tempColumn . "` = FROM_UNIXTIME(" . $column . ")");
        $statement->execute();
        DB::getSchemaBuilder()->table($tableName, function ($table) use($column) {
            $table->dropColumn($column);
        });
        DB::getSchemaBuilder()->table($tableName, function () use($tableName, $column, $tempColumn) {
            $pdo = DB::connection()->getPdo();
    ...
    {
        return DB::connection()->selectOne("select AES_DECRYPT(?, ?) as decrypted", array($cipherText, $key))->decrypted;
    }
    protected function encryptValue($text, $key)
    {
        return DB::connection()->selectOne("select AES_ENCRYPT(?, ?) as encrypted", array($text, $key))->encrypted;
    }
    protected function aesEncryptValue($text, $key)
    {
        $encryption = new \App\Security\Encryption\Aes();
    ...
    {
        return (new \App\Helpers\Pwd())->decrypt($value);
    }
    protected function encrypt($value)
    {
        return (new \App\Helpers\Pwd())->encrypt($value);
    }
    protected function asDateTime($value)
    {
        if ($value instanceof \App\Helpers\Carbon || $value instanceof \Carbon\Carbon) {
            return $value;
        }
        if ($value instanceof \DateTimeInterface) {
            return new \App\Helpers\Carbon($value->format("Y-m-d H:i:s.u"), $value->getTimeZone());
        }
        if (is_numeric($value)) {
            return \App\Helpers\Carbon::createFromTimestamp($value)->timezone(config('app.timezone'));
        }
        if (preg_match("/^(\\d{4})-(\\d{1,2})-(\\d{1,2})\$/", $value)) {
            return \App\Helpers\Carbon::createFromFormat("Y-m-d", $value)->timezone(config('app.timezone'))->startOfDay();
        }
        return \App\Helpers\Carbon::createFromFormat($this->getDateFormat(), $value)->timezone(config('app.timezone'));
    }
    ...
    public function getCustomFieldValuesAttribute()
    {
        ...
        if (!$this instanceof \App\Helpers\Customfield) {
            ...
            if (!is_null($customFieldType) && !is_null($customFieldRelId)) {
                $customFieldValues = $customFieldValues->filter(function (\App\Models\Customfieldsvalue $customFieldValue) use($customFieldType, $customFieldRelId) {
                    return $customFieldValue->customField->type === $customFieldType && $customFieldValue->customField->relatedId === $customFieldRelId;
                });
            } else {
                throw new \Exception("A model that supports custom fields must implement" . " getCustomFieldRelId() and getCustomFieldType() methods");
            }
        }
    ...
    protected function snake_case($value, $delimiter = '_')
    {
