...
class AbstractModelClient extends Authenticatable
{
    public $unique = array();
    ...
    public function getAttribute($key)
    {
        ...
        if ($isColumnMapped && in_array($originalKey, $this->getDates())) {
            ...
            if (in_array($dateValue, array("0000-00-00 00:00:00", "0000-00-00")) || empty($dateValue)) {
                $value = \App\Helpers\Carbon::createFromTimestamp(0, "UTC");
            } else {
    ...
    {
        if ($value instanceof \App\Helpers\Carbon) {
            return $value;
        }
        ...
        if (is_numeric($value)) {
            return \App\Helpers\Carbon::createFromTimestamp($value);
        }
        if (preg_match("/^(\\d{4})-(\\d{1,2})-(\\d{1,2})\$/", $value)) {
            return \App\Helpers\Carbon::createFromFormat("Y-m-d", $value)->startOfDay();
        }
        return \App\Helpers\Carbon::createFromFormat($this->getDateFormat(), $value);
    }
