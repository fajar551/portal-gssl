...
class Registrar
{
	...
	public function RegSaveHost($params){
		
		$values = $this->RegCallFunctionCreateHost($params, "SaveHostNameservers");
		...
		$data=\App\Models\Domain::find($params["domainid"]);
		$response = json_decode($values->getContent(), true);
		$userid=$data->userid;
		if ($response['code'] === 200 && $response['message'] === 'Successfully proccess the request!') {
			LogActivity::Save("Domain Registrar Command: Create Host Nameservers - Successful", $userid, $params["domainid"]);
		} else {
			LogActivity::Save("Domain Registrar Command: Create Host Nameservers - Failed", $userid, $params["domainid"]);
		}
		return $values;
	}

	public function RegDeleteHost($params){
		
		$values = $this->RegCallFunctionDeleteHost($params, "DeleteHostNameservers");
		...
		if ($response['data'] === true && $response['code'] === 200) {
			LogActivity::Save("Domain Registrar Command: Delete Host Nameservers - Successful", $userid, $params["domainid"]);
		} elseif ($response['error']) {
			LogActivity::Save("Domain Registrar Command: Delete Host Nameservers - Failed", $userid, $params["domainid"]);
		} else {
			LogActivity::Save("Domain Registrar Command: Delete Host Nameservers - Failed", $userid, $params["domainid"]);
		}
	...
	public function RegGetHost($params){
		
		$values = $this->RegCallFunctionGetNameServer($params, "GetHostNameservers");
	...
	public function RegCallFunctionCreateHost($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallFunctionDeleteHost($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallFunctionGetNameServer($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallFunctionEpp($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallFunctionDetails($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallUpdateContact($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallUpdateNameservers($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallFunctionRenew($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallFunctionLock($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegCallFunctionUnlock($params, $function, $noarr = false)
    {
		$registrar = $params["registrar"];
		$module = new self();
        $module->setDomainID($params["domainid"]);
	...
	public function RegRenewDomain($params = [])
	{
		...
		$params["domainObj"] = $domainObj;
		$values = $this->RegCallFunctionRenew($params, "RenewDomain");
		// if (!is_array($values)) {
		// 	return false;
		// }
		// if (isset($values["na"])) {
	...
	public function RegLockDomain($params = [])
	{
		$domainid = $params["domainid"];
		$data=\App\Models\Domain::find($domainid);
		...
		$params["domainObj"] = $domainObj;
		$values = $this->RegCallFunctionLock($params, "GetRegistrarLock");
		return $values;

	}

	public function RegUnlockDomain($params = [])
	{
		$domainid = $params["domainid"];
		$data=\App\Models\Domain::find($domainid);
		...
		$params["domainObj"] = $domainObj;
		$values = $this->RegCallFunctionUnlock($params, "GetRegistrarUnlock");
		return $values;
	...
	{
		$values = $this->regcallfunctionEpp($params, "GetEPPCode");
		if (!$values) {
			return false;
		}
	...
	public function RegGetDomainDetail($params)
	{
		$values = $this->RegCallFunctionDetails($params, "GetDomainDetails");
		if (!$values) {
			return false;
		}
	...
	public function RegUpdateContactDetails($params)
	{
		$values = $this->RegCallUpdateContact($params, "UpdateContactDetails");
		if (!$values) {
			return false;
		}
	...
	public function RegUpdateNameservers($params)
	{
		$values = $this->RegCallUpdateNameservers($params, "SaveNameservers");
		if (!$values) {
			return false;
		}
