...
class General extends \App\Helpers\Emailer
{
    protected function getEntitySpecificMergeData($userId, $extra)
    {
        if (in_array($this->message->getTemplateName(), array("Quote Delivery with PDF"))) {
            ...
            $this->message->addStringAttachment(\Lang::get("quotefilename") . $this->getExtra("quote_number") . ".pdf", $this->getExtra("quoteattachmentdata"));
        } else {
            $this->setRecipient($userId);
        }
    }
