...
class AbstractTask extends \Illuminate\Database\Eloquent\Model implements \App\Cron\DecoratorItemInterface, TaskInterface
{
    ...
    public static function boot()
    {
        ...
        static::created($alwaysHaveStatusRelationship);
    }
    ...
    public function anticipatedNextRun(\App\Helpers\Carbon $date = NULL)
    {
        if (is_null($date)) {
    ...
    protected function anticipatedNextMonthlyRun($checkDay, \App\Helpers\Carbon $date = NULL)
    {
        $checkDate = \App\Helpers\Carbon::now()->second("00");
        $nextMonth = \App\Helpers\Carbon::now()->second("00")->startOfMonth()->addMonth()->hour($checkDate->format("H"))->minute($checkDate->format("i"));
        ...
        if (is_null($date)) {
            $date = \App\Helpers\Carbon::now()->second("00");
        } else {
            $date = $date->copy();
        }
        if ($date->isFuture()) {
    ...
    {
        return \App\Helpers\Carbon::now()->startOfDay();
    }
