...
trait RegisterTrait
{
    ...
    public function getLatestOutputs(array $outputKeys = array())
    {
        ...
        return (new \App\Log\Register())->latestByNamespaces($namespaces, $this->id);
    }
    public function getOutputsSince(\App\Helpers\Carbon $since, array $outputKeys = array())
    {
        if (empty($outputKeys)) {
            $namespaceKeys = array_keys($this->getOutputKeys());
        } else {
        ...
        $namespaces = array_map($applyNamespace, $namespaceKeys);
        return (new \App\Log\Register())->sinceByNamespace($since, $namespaces, $this->id);
    }
