...
final class Queue extends \App\Models\AbstractModel
{
    ...
    {
        $key = sha1(config("portal.hash.cc_encryption_hash"));
    ...
    public function createDigestHash(\App\Scheduling\Contract\JobInterface $job)
    {
        $signatureKey = hash("sha256", config("portal.hash.cc_encryption_hash"), true);
    ...
    public function verifyDigestHash(\App\Scheduling\Contract\JobInterface $job)
    {
    ...
    protected function factoryJobFromClassName($className)
    {
        if (class_exists($className)) {
            try {
                $class = new $className();
                if ($class instanceof \App\Scheduling\Contract\JobInterface) {
                    return $class;
                }
            } catch (\Exception $e) {
                return null;
    ...
    protected function factoryJobInstance($name, $class, $method, $inputParams, $delay = 0)
    {
        ...
        if ($delay) {
            $availableAt = \App\Helpers\Carbon::now()->addMinutes($delay);
        } else {
            $availableAt = \App\Helpers\Carbon::now();
        }
    ...
    public function executeJob()
    {
        ...
        if (!$job) {
            throw new \App\Exceptions\Scheduling\Jobs\QueueException("An empty job or an unsuitable class queued for execution");
        }
        if (empty($methodName) || !method_exists($job, $methodName)) {
            throw new \App\Exceptions\Scheduling\Jobs\QueueException("Method does not exist in a job queued for execution");
        }
        ...
        $job->jobAvailableAt(new \App\Helpers\Carbon($this->available_at));
        $job->jobDigestHash($storedHash);
        $this->validateJob($job);
        if (!$this->verifyDigestHash($job)) {
            throw new \App\Exceptions\Scheduling\Jobs\QueueException("Job signature validation failed");
        }
        $job->{$methodName}(...$methodArguments);
    }
    public function validateJob(\App\Scheduling\Contract\JobInterface $job)
    {
        $msg = "";
        if (!$job->jobName()) {
        ...
        if ($msg) {
            throw new \App\Exceptions\Model\EmptyValue($msg);
        }
        return true;
    }
    public function addJob(\App\Scheduling\Contract\JobInterface $job, $replaceExisting = false)
    {
        $result = false;
        try {
            $queue = new static();
            ...
            \App\Helpers\LogActivity::Save("Exception thrown when adding job to queue" . " - " . $e->getMessage());
        }
        return $result;
    }
}
