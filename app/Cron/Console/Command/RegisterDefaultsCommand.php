...
class RegisterDefaultsCommand extends \Symfony\Component\Console\Command\Command
{
    ...
    {
        $io = new \App\Cron\Console\Style\TaskStyle($input, $output);
        $io->section("Delete Registered Tasks");
        $proceed = $io->confirm("Delete all current registered tasks?", false);
        if ($proceed) {
            $oldTasks = \App\Scheduling\Task\AbstractTask::all();
            $oldTasks = $oldTasks->isLevel(\App\Scheduling\Task\TaskInterface::ACCESS_USER);
        ...
        if ($proceed) {
            $tasks = \App\Scheduling\Task\AbstractTask::all();
            self::resetNextRun($tasks, \App\Helpers\Carbon::now());
        }
        $io->section("Last Daily Cron");
        $proceed = $io->confirm("Reset Last Daily Cron Invocation?", false);
        if ($proceed) {
            \App\Helpers\Cfg::setValue("lastDailyCronInvocationTime", "");
        }
    ...
    public static function registerAnyNonSystemTask(\App\Cron\Console\Style\TaskStyle $io = NULL)
    {
        $instances = array();
        $finder = (new \Symfony\Component\Finder\Finder())->files()->in(app_path("Cron".DIRECTORY_SEPARATOR."Task"))->name("*.php");
        foreach ($finder as $item) {
            $filename = $item->getBasename(".php");
            if (strpos($filename, "Abstract") !== false) {
                continue;
            }
            $classname = "App\\Cron\\Task\\" . $filename;
            if ($io && $io->isDebug()) {
            ...
            if (!$instance instanceof \App\Scheduling\Task\TaskInterface) {
                if ($io && $io->isDebug()) {
                    $io->text("- Class " . $classname . " is not of TaskInterface");
                }
                continue;
            }
            if ($instance->getAccessLevel() != \App\Scheduling\Task\TaskInterface::ACCESS_USER) {
                if ($io && $io->isDebug()) {
        ...
        return new \App\Scheduling\Task\Collection($instances);
    }
    public static function resetNextRun(\App\Scheduling\Task\Collection $tasks, \App\Helpers\Carbon $nextRunTime)
    {
