...
abstract class AbstractCronCommand extends \Symfony\Component\Console\Command\Command
{
    ...
    {
        \App\Helpers\Hooks::run_hook("AfterCronJob", array());
        if ($this->io->isVerbose()) {
    ...
    {
        return new \App\Scheduling\Task\Collection(array());
    }
    ...
    protected function executeCollection(\App\Scheduling\Task\Collection $tasks)
    {
        $totalQueuedTasks = $tasks->count();
        ...
        if ($totalQueuedTasks) {
            ...
            foreach ($tasks as $task) {
                ...
                if (60 < $task->getFrequencyMinutes() || $this->io->isDebug()) {
                    \App\Helpers\LogActivity::Save("Automated Task: Starting " . $task->getName());
                }
                try {
                    \App\Helpers\Hooks::run_hook("PreAutomationTask", array("task" => $task), true);
                    ...
                    try {
                        \App\Helpers\Hooks::run_hook("PostAutomationTask", array("task" => $task, "completed" => $completed), true);
                    } catch (\Exception $e) {
                    }
                }
            }
