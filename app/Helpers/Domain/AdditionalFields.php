...
class AdditionalFields
{
    ...
    protected function loadFieldsData()
    {
        ...
        try {
            ...
            if (Auth::guard('web')->check()) {
                $domain->where("userid", Auth::guard('web')->user()->id);
            }
            $domain = $domain->first();
            if (!$domain) {
                $extensionData = \App\Models\Extension::where("extension", $this->getTLD())->where("autoreg", "!=", "")->firstOrFail();
    ...
    {
        $values = array();
		$getAdditionalFields=\App\Models\Domainsadditionalfield::select('name','value')->where('domainid',$domainID)->get();
		foreach($getAdditionalFields as $data){
