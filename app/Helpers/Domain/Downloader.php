    ...
    public function getCertificate($domain)
    {
        ...
        if (curl_errno($ch)) {
            ...
            if (in_array($errorNumber, array(CURLE_SSL_PEER_CERTIFICATE, CURLE_SSL_CACERT))) {
                throw new \App\Exceptions\Information("No SSL");
            }
        }
        curl_close($ch);
        if (isset($certInfo[0]) && is_array($certInfo[0])) {
            return new \App\Helpers\Domain\Certificate($certInfo[0]);
        }
        throw new \Exception("Unable to retrieve certificate data");
    }
}
