...
class ClientArea
{
    ...
    public function requireLogin()
    {
		...
		session()->put("loginurlredirect", html_entity_decode($_SERVER["REQUEST_URI"]));
        if (session("2faverifyc")) {
            ...
            // } else {
            //     if (Request::input("incorrect")) {
            //         $this->assign("incorrect", true);
            //     }
            // }
            // $twofa = new TwoFactorAuthentication();
            // if ($twofa->setClientID(session("2faclientid"))) {
            //     if (!$twofa->isActiveClients() || !$twofa->isEnabled()) {
            //         Session::destroy();
            //         redir();
            //     }
            //     if (Request::input("backupcode")) {
            ...
            $this->assign("formaction", route('login'));
            $this->assign("incorrect", (bool) Request::input("incorrect"));
            $this->assign("ssoredirect", (bool) Request::input("ssoredirect"));
            // $this->assign("captcha", new Utility\Captcha());
            // $this->assign("captchaForm", Utility\Captcha::FORM_LOGIN);
			// TODO: this
			$this->assign("captcha", "");
            $this->assign("captchaForm", "");
            // $this->assign("invalid", sessionAndDelete("CaptchaError"));
            $this->assign("invalid", session("CaptchaError"));
        }
        $this->output();
        // exit;
    }
    ...
    public function outputWithoutExit()
    {
        ...
        $this->assign("adminLoggedIn", (bool) session("adminid"));
