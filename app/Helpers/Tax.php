...
class Tax
{
	...
	protected function validateTaxBase($taxBase)
	{
		if (!is_numeric($taxBase) || is_nan($taxBase)) {
			throw new \App\Exceptions\Billing\BillingException("Invalid tax base: " . (string) $taxBase);
		}
	}
	...
	protected function validateTaxPercentage($percentage)
	{
		if (!is_numeric($percentage) || is_nan($percentage) || 100 < $percentage) {
			throw new \App\Exceptions\Billing\BillingException("Invalid tax percentage: " . (string) $percentage);
		}
	}
	...
	protected function validateTaxLevelPercentages($level1Percentage, $level2Percentage)
	{
		if (100 < $level1Percentage + $level2Percentage) {
			throw new \App\Exceptions\Billing\BillingException("Combined L1 and L2 tax percentage is over 100%");
		}
	}
	...
	protected function recalculate()
	{
		...
		if ($this->isInclusive) {
			if ($this->isCompound) {
				$this->level2TaxTotal = Format::AsCurrency($this->taxBase - $this->taxBase / (1 + $level2Mult));
				$preLevel2TaxBase = $this->taxBase - $this->level2TaxTotal;
				$this->level1TaxTotal = Format::AsCurrency($preLevel2TaxBase - $preLevel2TaxBase / (1 + $level1Mult));
			} else {
				$preTaxBase = $this->taxBase / (1 + $level1Mult + $level2Mult);
				$this->level1TaxTotal = Format::AsCurrency($preTaxBase * $level1Mult);
				$this->level2TaxTotal = Format::AsCurrency($preTaxBase * $level2Mult);
			}
			$this->totalBeforeTaxes = $this->taxBase - $this->level1TaxTotal - $this->level2TaxTotal;
		} else {
			$this->totalBeforeTaxes = $this->taxBase;
			$this->level1TaxTotal = Format::AsCurrency($this->totalBeforeTaxes * $level1Mult);
			$level2Base = $this->isCompound ? $this->totalBeforeTaxes + $this->level1TaxTotal : $this->totalBeforeTaxes;
			$this->level2TaxTotal = Format::AsCurrency($level2Base * $level2Mult);
		}
		return $this;
	}
