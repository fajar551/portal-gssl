...
class Addon
{
    ...
    {
        $addon = \App\Models\Addon::with("productAddon")->where("id", $this->id);
        if ($this->userId) {
    ...
    {
        return $this->getData("product_addon", "module");
    }
    ...
    public function moduleCall($function, $vars = array())
    {
        ...
        $hookResults = \App\Helpers\Hooks::run_hook("PreModule" . $hookFunction, array("params" => $vars));
        try {
