...
class AdditionalFields
{
    ...
    /**
     * Create a new AdditionalFields instance.
     *
    ...
    protected function fetch($file)
    {
        ...
        if (file_exists($this->getResourcePath() . $file)) {
            if (is_null($this->countries)) {
                $this->countries = new \App\Helpers\Country();
            }
    ...
    protected function loadFieldsData()
    {
        ...
        try {
            ...
            $domain = DB::table("tbldomains")->where("domain", $this->activeDomain)->where("registrar", "!=", "");
            if ($auth) {
                $domain->where("userid", (int) $auth->id);
            }
            $domain = $domain->first();
            if (!$domain) {
                $extensionData = \App\Models\Domainpricing::where("extension", $this->getTLD())->where("autoreg", "!=", "")->firstOrFail();
                $domainRegistrar = $extensionData->autoreg;
            } else {
        ...
        if ($domainRegistrar) {
            ...
            // if ($registrar->load($domainRegistrar)) {
                $result = $registrar->loadModule($domainRegistrar, "AdditionalDomainFields", array("tld" => substr($this->getTLD(), 1), "idprotection" => $idProtection, "type" => $this->domainType, "fields" => $fields));
    ...
    {
        $values = array();
        $result = \App\Models\Domainsadditionalfield::where('domainid', $domainID)->get();
    ...
    public function saveToDatabase($domainID, $creating = true)
    {
        ...
        foreach ($this->getFields() as $fieldKey => $values) {
            $name = $this->getConfigValue($fieldKey, "Name");
            $value = $this->getFieldValue($fieldKey);
            $additionalField = \App\Models\Domainsadditionalfield::firstOrNew(array("domainid" => $domainID, "name" => $name));
            ...
            if (!$userId) {
                $userId = $additionalField->domain->userid;
            }
        }
        if (count($changes) && !$creating) {
            LogActivity::Save("Modified Domain Additional Fields - " . implode(", ", $changes) . " - User ID: " . $userId . " - Domain ID: " . $domainID, $userId);
        }
    }
