...
class SearchResult extends \App\Helpers\Domain\Domain
{
    protected $score = 1;
    ...
    public static function factoryFromDomain(\App\Helpers\Domain\Domain $domain)
    {
    ...
    public function toArray()
    {
        ...
        } catch (\App\Exceptions\Domains\Pricing\ContactUs $e) {
            $pricing = "ContactUs";
            $shortestPeriod = 0;
        } catch (\App\Exceptions\Domains\Pricing\NoSale $e) {
    ...
    {
        if (\App\Helpers\Cfg::getValue("PremiumDomains") && $this->isPremiumDomain()) {
    ...
    protected function calculatePremiumPricing()
    {
        try {
            ...
            $registrarCurrencyId = DB::table("tblcurrencies")->where("code", "=", $this->getPremiumCostPricing()["CurrencyCode"])->value("id");
            if (!$registrarCurrencyId) {
                throw new \Exceptions("Domain registrar currency not available for conversion");
            }
            $auth = Auth::guard('web')->user;
            $clientCurrency = \App\Helpers\Format::getCurrency($auth ? $auth->id : 0, \Session::get("currency"));
            ...
            foreach ($this->getPremiumCostPricing() as $registerType => &$premiumPrice) {
                ...
                $premiumPricing[$registerType] = \App\Helpers\Invoice::convertCurrency($premiumPrice, $registrarCurrencyId, $clientCurrency["id"]);
            }
            $registerTransferKey = "register";
            if (array_key_exists("transfer", $premiumPricing)) {
                $registerTransferKey = "transfer";
            }
            $hookReturns = \App\Helpers\Hooks::run_hook("PremiumPriceOverride", array("domainName" => $this->getRawDomain(), "tld" => $this->getTopLevel(), "sld" => $this->getSecondLevel(), $registerTransferKey => $premiumPricing[$registerTransferKey], "renew" => $premiumPricing["renew"]));
            $skipMarkup = false;
            foreach ($hookReturns as $hookReturn) {
                if (array_key_exists("noSale", $hookReturn) && $hookReturn["noSale"] === true) {
                    throw new \App\Exceptions\Domains\Pricing\NoSale();
                }
                if (array_key_exists("contactUs", $hookReturn) && $hookReturn["contactUs"] === true) {
                    throw new \App\Exceptions\Domains\Pricing\ContactUs();
                }
            ...
            foreach ($premiumPricing as $registerType => &$price) {
                ...
                $pricingArray[1][$registerType] = \App\Helpers\Format::formatCurrency($price);
            }
