...
// HOTFIX: need rwrite
class FileUpload extends \App\Helpers\File
{
    ...
    {
        // TODO: if ($nameOrFile instanceof \Zend\Diactoros\UploadedFile) {
        //     $this->uploadedFile = $nameOrFile;
        // } else {
            if (!isset($_FILES[$nameOrFile])) {
                throw new \App\Exceptions\File\NotUploaded("Check name and key parameters.");
            }
            if (is_numeric($key)) {
            ...
            if (!$this->isUploaded()) {
                throw new \App\Exceptions\File\NotUploaded(\Lang::get("client.filemanagement.nofileuploaded"));
            }
            if (!$this->isFileNameSafe($this->getCleanName())) {
                throw new \Exception(\Lang::get("client.filemanagement.invalidname"));
            }
        // TODO: }
    }
    ...
    public static function isExtensionAllowed($filename)
    {
        ...
        $extensionArray = explode(",", strtolower(Cfg::get("TicketAllowedFileTypes")));
