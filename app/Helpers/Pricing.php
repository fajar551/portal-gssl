...
class Pricing
{
    ...
    public function loadPricing($type, $relid, $currency = NULL)
    {
        ...
        if (is_null($this->currency)) {
            $this->currency = (new \App\Helpers\AdminFunctions())->getCurrency();
        }
        $result = \App\Models\Pricing::where('type', $type)->where('currency', (int) $this->currency["id"])->where('relid', (int) $relid)->get();
        // HOTFIX: if (is_array($data)) {
        if ($result) {
            $data = $result->toArray();
            $this->data = $data;
        } else {
    ...
    protected function getCycleData($cycle, $months = 0)
    {
        ...
        $configoptions = \App\Helper\ConfigOptions::getCartConfigOptions($this->getRelID(), array(), $cycle, "", true);
        ...
        if (0 < $months) {
            $breakdown = array("monthly" => new \App\Helpers\FormatterPrice($price / $months, $this->currency), "yearly" => 12 <= $months ? new \App\Helpers\FormatterPrice($price / ($months / 12), $this->currency) : null);
        } else {
            $breakdown = array();
        }
        return array("cycle" => $cycle, "setupfee" => new \App\Helpers\FormatterPrice($setupfee, $this->currency), "price" => new \App\Helpers\FormatterPrice($price, $this->currency), "breakdown" => $breakdown);
    }
