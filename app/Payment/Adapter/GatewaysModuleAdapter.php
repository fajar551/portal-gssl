...
class GatewaysModuleAdapter extends AbstractAdapter
{
    ...
    protected function detectSolutionTypeFromCapabilities()
    {
        ...
        if (!$this->refundCapable) {
            $type = \App\Payment\Solutions::TYPE_ALTERNATE;
        } else {
            if ($this->captureCapable && $this->linkCapable) {
                $type = \App\Payment\Solutions::TYPE_MULTI;
            } else {
                if ($this->linkCapable && !$this->captureCapable) {
                    $type = \App\Payment\Solutions::TYPE_ALTERNATE;
                } else {
                    if ($this->captureCapable) {
                        $type = \App\Payment\Solutions::TYPE_GATEWAY;
                    } else {
                        throw new \App\Payment\Exception\InvalidModuleException(sprintf("Payment solution module '%s' does not implement either a capture or link function", $name));
                    }
                }
            }
        }
    ...
    protected function getConfigurationFromDefinedFunctions()
    {
        ...
        if (function_exists($name . "_config")) {
            ...
            if (function_exists($name . "_activate")) {
                ...
                throw new \App\Payment\Exception\InvalidModuleException(sprintf("Payment solution module '%s' does not implement a configuration function", $name));
            }
        }
        return $config;
    }
