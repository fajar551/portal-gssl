    ...
    public function getAdapterFromFile(\SplFileInfo $resource)
    {
        ...
        if (class_exists($resource_classname)) {
            ...
            if (!$adapter instanceof Adapter\AdapterInterface) {
                throw new Exception\InvalidModuleException(sprintf("Payment solution module class '%s' does not implement %s", $resource_classname, "App\\Payment\\Adapter\\AdapterInterface"));
            }
        } else {
