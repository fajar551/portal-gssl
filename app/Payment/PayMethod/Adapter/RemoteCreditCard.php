...
class RemoteCreditCard extends CreditCardModel implements \App\Payment\Contracts\RemoteTokenDetailsInterface
{
    ...
    {
        $client = new \App\Helpers\ClientClass($client->id);
        ...
        if ($gatewayCallResult["status"] == "success") {
            ...
            if (isset($gatewayCallResult["remoteToken"])) {
                ...
                if (array_key_exists("cardExpiry", $gatewayCallResult)) {
                    ...
                    if (!$expiry instanceof \App\Helpers\Carbon) {
                        $expiry = \App\Helpers\Carbon::createFromFormat("my", $expiry)->endOfMonth();
                    }
