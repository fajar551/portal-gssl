...
jQuery(document).ready(function(){
    ...
    jQuery("#frmConfigureProduct").submit(function(e) {
        ...
        $.ajax({
            url: route('cart'),
            type: 'post',
            data: 'ajax=1&a=confproduct&' + jQuery("#frmConfigureProduct").serialize(),
            success: function(data) {
                // console.log(jQuery("#frmConfigureProduct").serializeArray());
                if (data) {
                    jQuery("#btnCompleteProductConfig").html(btnOriginalText);
                    jQuery("#containerProductValidationErrorsList").html(data);
                    jQuery("#containerProductValidationErrors").removeClass('hidden').show();
                    ...
                    window.location = route('cart')+'?a=confdomains';
                }
            },
        });
    });
    ...
    jQuery(".addon-products").on('ifChecked', '.panel-addon input', function(event) {
        ...
        $activeAddon.find(".panel-addon-customfields").removeClass("d-none").click(false);
    ...
    jQuery(".addon-products").on('ifUnchecked', '.panel-addon input', function(event) {
        var $activeAddon = jQuery(this).parents('.panel-addon');
        $activeAddon.find(".panel-addon-customfields").addClass("d-none");
            ...
            otherDomain.done(function(data) {
                ...
                jQuery.each(data.result, function(index, result) {
                    if (result.status === true) {
                        window.location = route('cart')+'?a=confproduct&i=' + result.num;
                    } else {
                        jQuery('.domain-lookup-primary-loader').hide();
...
function recalctotals() {
    ...
    var post = $.ajax({
        url: route('cart'),
        type: 'post',
        data: 'ajax=1&a=confproduct&calctotal=true&'+jQuery("#frmConfigureProduct").serialize(),
    });
    post.done(
        function(data) {
