...
@section('title')
    <title>{{ Cfg::getValue('CompanyName') }} -  Client Quotes</title>
@endsection
                                            ...
                                            <div class="col-lg-12">
                                                <div class="row">
                                                    <div class="col-lg-12 d-flex flex-row-reverse mb-2">
                                                        <a href="add-new-quote.html">
                                                            <button class="btn btn-outline-success align-items-center d-flex">
                                                                <i class="ri-add-fill mr-2"></i> Create New Quote
                                                            </button>
                                                        </a>
                                                    </div>
                                                </div>
    ...
    <script>
        ...
        $(() => {
            ...
            $('body').on('click', '.act-delete', function() {
                deleteQuote($(this).attr('data-id'));
            });
        });

        const dtClientQuotes = () => {
            dtTableQuotes = $('#dt-quotes').DataTable({
                ...
                ajax: {
                    url: "{!! route('admin.pages.clients.viewclients.clientsummary.dtClientQuote') !!}",
                    type: "GET",
                    data: (data) => {
        ...
        const deleteQuote = async (id) => {
            const url = "{!! route('admin.pages.clients.viewclients.clientquotes.delete') !!}";
            const payloads = { 
                id, 
